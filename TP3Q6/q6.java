...
public class q6{
    ...
    public static void quickSort(int esq, int dir, Lista lista) throws Exception{
        int i = esq, j = dir;
        Serie pivo = lista.array[(dir + esq) / 2];

        while(i <= j){
            while(lista.array[i].getPais().compareTo(pivo.getPais()) < 0 ||
            (lista.array[i].getPais().compareTo(pivo.getPais()) == 0 &&
            lista.array[i].getNome().compareTo(pivo.getNome()) < 0 )){
                i++;
            }

            //System.out.println("antes do swap: " + lista.array[i].getNome()  + " | " + lista.array[j].getNome()  + " | " + pivo.getNome() + "\n");
            
            while(lista.array[j].getPais().compareTo(pivo.getPais()) > 0 ||
            (lista.array[j].getPais().compareTo(pivo.getPais()) == 0 &&
            lista.array[j].getNome().compareTo(pivo.getNome()) > 0 )){
                j--;
            }
            ...
            if(i <= j){
                ...
                Serie aux = lista.array[i]; lista.array[i] = lista.array[j]; 
                lista.array[j] = aux;
