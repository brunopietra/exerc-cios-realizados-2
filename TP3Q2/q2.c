...
//metodo de ordenacao, cuja chave primaria e pais
...
void selecao(Lista *lista, int i, int *comp, int *troc){
    if (i < (lista->n - 1)) {
      int menor = i;
      for (int j = (i + 1); j < lista->n; j++){
         if (strcmp(lista->array[menor].pais, lista->array[j].pais) == 0){
         ...
         else if (strcmp(lista->array[menor].pais, lista->array[j].pais) > 0){
             menor = j;
             *troc = *troc + 1;
             *comp = *comp + 1;
         }
      ...
      swap(menor, i, lista);
      selecao(lista, i + 1, comp, troc);
    }
    
    
}

void chamaSelecao(Lista *lista, int *comp, int *troc){
    selecao(lista, 0, comp, troc);
}
...
void gerarArq(double time, int comp, int troc){
    ...
    fp = fopen("matricula_sequencialReferencial.txt","w");
