...
class tabHash{
    ...
    public boolean pesquisar(String nomeSerie){
        ...
        if(resp == false){
            resp = tabReserva.pesquisar(nomeSerie);
        }
    ...
    public void mostrar(){
        //System.out.println("Elementos da tabela principal: ");
        for(int i = 0; i < tabela.length; i++){
            if(tabela[i] != null){
                //System.out.printf(" %s || pos %d || ASCII: %d\n", tabela[i].getNome(), i, convToASCII(tabela[i].getNome()));
            }
...
class Lista {
    ...
    public boolean pesquisar(String nomeSerie){
        ...
        for(int i = 0; i < n; i++){
            if(tabela[i].getNome().compareTo(nomeSerie) == 0){
                resp = true;
                i = n;
            }
        }
    ...
    public void mostrar(){
        //System.out.println("Elementos da tabela reserva: ");
        for(int i = 0; i < n; i++){
            //System.out.printf(" %s || pos %d\n", tabela[i].getNome(), i);
...
public class q6{
    ...
    public static void gerarArq(long time, Comp comparacoes) throws Exception{
        try{
            PrintWriter writer = new PrintWriter("matricula_hashReserva.txt", "UTF-8");
